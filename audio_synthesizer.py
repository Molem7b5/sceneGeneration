...
class AudioSynthesizer(object):
    def __init__(
        ...
        for nr in range(self._nb_rooms):
            # self._room_names.append(self._rirdata[nr][0][0][0]) # dont have room names
            self._room_names.append('room name')
    ...
    def synthesize_mixtures(self):
        ...
        for mic_arr in range(self._nb_mic_arrays):
            print('Generating scene audio for fold {}'.format(mic_arr + 1))

            rooms = self._mixtures[mic_arr][0]['roomidx']
            nb_rooms_in_fold = len(rooms)
            for nr in range(nb_rooms_in_fold):

                nroom = rooms[nr]
                nb_mixtures = len(self._mixtures[mic_arr][nr]['mixture'])
