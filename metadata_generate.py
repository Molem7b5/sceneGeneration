...
class MetadataSynthesizer(object):
    def __init__(
        ...
        self._metadata_path = params['mixturepath'] + '/' + 'metadata' # currently no meta data for my IRs
        ...
        self._mixture_setup['fs_mix'] = 48000  # fs of RIRs
        self._mixture_setup['mixture_duration'] = params['mixture_duration']
        self._nb_mic_arrays = db_config._nb_mic_arrays
        ...
        self._rirdata = db_config._rirdata # no meta data file at present for my data
    ...
    def create_mixtures(self):
        self._mixtures = []
        foldlist = []
        rirdata2room_idx = {1: 0, 2: 1, 3: 2, 4: 3, 5: 4, 6: 5, 7: 6, 8: 7, 9: 8, 10: 9, 11: 10}  # room numbers in the rirdata array

        for nfold in range(self._mixture_setup['nb_folds']):
        # for mic_array in range(self._nb_mic_arrays):
            print('Generating metadata for mic array {}'.format(str(nfold + 1))) # change this to f string

            foldlist_nff = {}
            rooms_nf = np.array(self._mixture_setup['rooms2folds'][nfold]) # have all mics in mixture set up to use all mics with all folds
            rooms_nf = rooms_nf[rooms_nf > 0]
            nb_rooms_nf = self._nb_mic_arrays
            ...
            for nm in range(self._nb_mic_arrays):
                fold_mixture = {'mixture': []}
                fold_mixture['mic_array_idx'] = nm
                # nroom = rooms_nf[nr]
                # print('Room {} \n'.format(nroom + 1))
                #n_traj = np.shape(self._rirdata[rirdata2room_idx[nroom]][0][2])[0]  # number of trajectories
                traj_doas = []

                # My dataset does not have moving objects
                ...
                for nmix in range(nb_mixtures_per_fold_per_room):
                    ...
                    for layer in range(self._mixture_setup['nOverlap']):
                        ...
                        while event_time_in_layer < self._total_event_time_per_layer:
                            ...
                            if (trimmed_event_length > self._trim_threshold) and (trimmed_event_length > np.floor(ons*10.)/10.).any():
                            ...
                            for nl in range(nb_samples_in_layer):
                                ...
                                event_class_nl = int(foldlist_nf['class'][event_nl]) # probably don't need this
                                ...
                                if nl == 0 and layer == 0:
                                    ...
                                    nth_mixture['doa_azel'] = [self._rirdata[riridx]]
                                    ...
                                    nth_mixture['doa_azel'].append(self._rirdata[riridx])
            ...
            for nr in range(nb_rooms):
                ...
                for nmix in range(nb_mixtures):
                    ...
                    for nev in range(nb_events):
                        ...
                        doa_azel = mixture_nm['doa_azel'][nev]
                        if doa_azel.ndim == 1:
                            doa_azel = doa_azel[np.newaxis,:]
